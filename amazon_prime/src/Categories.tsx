import './Style/Categories.css'
function Categories() {
    return (
        <>
            <div className='whole2'>
                <p className='Categories'>Categories</p>
                <p className='titles'>Genres</p>
                <div className='Genres'>
                    <p className='box1'>Action and adventure</p>
                    <p className='box2'>Anime</p>
                    <p className='box3'>Comedy</p>
                    <p className='box4'>Documentary</p>
                    <p className='box5'>Drama</p>
                    <p className='box6'>Fantasy</p>
                    <p className='box7'>Horror</p>
                    <p className='box8'>Kids</p>
                    <p className='box9'>Mystery and thillers</p>
                    <p className='box10'>Romance</p>
                    <p className='box11'>science fiction</p>
                </div>
                 <p className='titles'>Featured Collections</p>
                <div className='Featured'>
                    <p>Home premiere</p>
                    <p>New Releases</p>
                    <p>MX Player</p>
                    <p>Critically acclaimed</p>
                    <p>Kids</p>
                </div>
                <img className='foot-img foot1'src="https://m.media-amazon.com/images/G/01/digital/video/acquisition/logo/pv_logo_white._CB548637580_.png" alt="Amazon"></img>
                <div className='bottom'>
                    <p>Terms and Privacy Notice</p>
                    <p>Send us feedback</p>
                    <p>Help</p>
                    <p>© 1996-2026, Amazon.com, Inc. or its affiliates</p>
                </div>
            </div>
        </>
    )
}
export default Categories